" File: context_envinsert.vim
" Author: Michael Goerz (goerz AT physik DOT fu MINUS berlin DOT de)
" Version: 0.9
" Copyright: Copyright (C) 2010 Michael Goerz
...
"
" Description: 
"    This maps the <leader>i key to a ConTeXt environment inserter. When you
"    press <leader>i while the cursor is on or directly after a word, that
"    word is transformed into a ConTeXt environment tag, When the cusor is not
...
function! ConTeXt_PutEnv(com)
    ...
    if matchstr(a:com, '^.') == '\'
        ...
        let base_name = a:com
        let lastletter = matchstr(base_name, '.$')
        if (lastletter == ']')
            let base_name = strpart(base_name, 0, match(base_name, '['))
        endif
        return IMAP_PutTextWithMovement('\start'.a:com.'<++>\stop'.base_name)
    endif
endfunction 

function! ConTeXt_CreateEnv()
	if getline('.') == ''
		let com = input('Choose an environment or math function to insert: ')
		if com != ''
			return ConTeXt_PutEnv(com)
		else
			return ''
		endif
	else
